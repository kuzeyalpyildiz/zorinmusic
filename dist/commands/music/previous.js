...
const command = {
    ...
    async execute(interaction) {
        ...
        if (!queue) {
            await interaction.editReply({ embeds: [(0, embeds_1.createErrorEmbed)('No active queue!')] });
            setTimeout(() => {
                interaction.deleteReply().catch(() => { });
            }, 5000);
            return;
        ...
        if (!previousTrack) {
            await interaction.editReply({ embeds: [(0, embeds_1.createErrorEmbed)('No previous track found in history.')] });
            setTimeout(() => {
                interaction.deleteReply().catch(() => { });
            }, 5000);
            return;
        ...
        await interaction.editReply({ embeds: [(0, embeds_1.createSuccessEmbed)(`⏮️ Playing previous track: **${previousTrack.info.title}**`)] });
        setTimeout(() => {
            interaction.deleteReply().catch(() => { });
        }, 5000);
    },
