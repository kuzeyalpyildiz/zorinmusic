...
const command = {
    ...
    async execute(interaction) {
        ...
        if (!queue) {
            await interaction.reply({ embeds: [(0, embeds_1.createErrorEmbed)('I am not in a voice channel!')], ephemeral: true });
            return;
        }
        client.destroyPlayer(interaction.guildId);
        await interaction.reply({ embeds: [(0, embeds_1.createSuccessEmbed)('👋 Left the voice channel.')] });
    ...
    async executePrefix(message, args) {
        ...
        if (!queue) {
            const err = await message.reply({ embeds: [(0, embeds_1.createErrorEmbed)('I am not in a voice channel!')] });
            setTimeout(() => {
                err.delete().catch(() => { });
                message.delete().catch(() => { });
            }, 5000);
        ...
        const reply = await message.reply({ embeds: [(0, embeds_1.createSuccessEmbed)('👋 Left the voice channel.')] });
        setTimeout(() => {
            reply.delete().catch(() => { });
            message.delete().catch(() => { });
        }, 5000);
