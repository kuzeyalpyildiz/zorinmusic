...
const command = {
    ...
    async execute(interaction) {
        ...
        if (pos > queue.tracks.length) {
            await interaction.reply({ embeds: [(0, embeds_1.createErrorEmbed)(`Invalid position! There are only ${queue.tracks.length} tracks in the queue.`)], ephemeral: true });
            return;
        }
        const removedTrack = queue.removeTrack(pos - 1);
        if (!removedTrack) {
            await interaction.reply({ embeds: [(0, embeds_1.createErrorEmbed)('Failed to remove track.')], ephemeral: true });
            return;
        }
        await interaction.reply({ embeds: [(0, embeds_1.createSuccessEmbed)(`Removed **${removedTrack.info.title}** from the queue.`)] });
