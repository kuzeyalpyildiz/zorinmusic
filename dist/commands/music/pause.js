...
const command = {
    ...
    async execute(interaction) {
        ...
        if (queue.paused) {
            await interaction.reply({ embeds: [(0, embeds_1.createErrorEmbed)('The track is already paused!')], ephemeral: true });
            return;
        }
        queue.paused = true;
        queue.player.setPaused(true);
        await interaction.reply({ embeds: [(0, embeds_1.createSuccessEmbed)('⏸️ Paused the current track.')] });
        setTimeout(() => {
