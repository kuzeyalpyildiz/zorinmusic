...
const command = {
    ...
    async execute(interaction) {
        ...
        if (!queue.paused) {
            await interaction.editReply({ embeds: [(0, embeds_1.createErrorEmbed)('The track is not paused!')] });
            setTimeout(() => {
                interaction.deleteReply().catch(() => { });
            }, 5000);
            return;
        ...
        await interaction.editReply({ embeds: [(0, embeds_1.createSuccessEmbed)('▶️ Resumed the current track.')] });
        setTimeout(() => {
            interaction.deleteReply().catch(() => { });
        }, 5000);
    },
