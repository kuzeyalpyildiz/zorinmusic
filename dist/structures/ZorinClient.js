...
class ZorinClient extends discord_js_1.Client {
    ...
    constructor() {
        ...
        // If primary node is different, place it first followed by fallback pool
        const nodes = [primaryNode, ...fallbackNodes.filter(n => n.url !== primaryNode.url)];
    ...
    getNode() {
        ...
        const node = connectedNode ?? [...this.shoukaku.nodes.values()][0];
        if (!node)
            throw new Error('No Lavalink nodes available. Please check node connectivity.');
        return node;
    }
    /**
    ...
    async createPlayer(guildId, channelId, shardId, textChannelId) {
        ...
        const queue = new MusicQueue_1.MusicQueue(player, textChannelId);
        this.queues.set(guildId, queue);
        // ── Player events ──
        player.on('start', async () => {
            if (!queue.current)
                return;
            // Cancel any pending 15-minute idle leave timer
    ...
    async start() {
        ...
        this.shoukaku.on('close', (name, code, reason) => {
            if (code === 4000) {
                console.warn(`[Lavalink] ⚠️ Node "${name}" rate-limited connection attempt (code 4000). Backing off for 15s…`);
            }
            else {
