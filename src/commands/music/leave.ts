...
const command: ZorinCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (!queue) {
            await interaction.editReply({ embeds: [createErrorEmbed('I am not in a voice channel!')] });
            setTimeout(() => {
                interaction.deleteReply().catch(() => {});
            }, 5000);
            return;
        ...
        await interaction.editReply({ embeds: [createSuccessEmbed('👋 Left the voice channel.')] });
        setTimeout(() => {
            interaction.deleteReply().catch(() => {});
        }, 5000);
    },
