...
const command: ZorinCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (!queue || queue.tracks.length < 2) {
            await interaction.reply({ embeds: [createErrorEmbed('Not enough tracks in the queue to shuffle!')], ephemeral: true });
            return;
        }

        queue.shuffle();

        await interaction.reply({ embeds: [createSuccessEmbed('🔀 Shuffled the queue!')] });
