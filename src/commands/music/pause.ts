...
const command: ZorinCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (queue.paused) {
            await interaction.editReply({ embeds: [createErrorEmbed('The track is already paused!')] });
            setTimeout(() => {
                interaction.deleteReply().catch(() => {});
            }, 5000);
            return;
        ...
        await interaction.editReply({ embeds: [createSuccessEmbed('⏸️ Paused the current track.')] });
        setTimeout(() => {
            interaction.deleteReply().catch(() => {});
        }, 5000);
    },
