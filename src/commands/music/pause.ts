...
const command: ZorinCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (queue.paused) {
            await interaction.reply({ embeds: [createErrorEmbed('The track is already paused!')], ephemeral: true });
            return;
        }

        queue.paused = true;
        queue.player.setPaused(true);
        await interaction.reply({ embeds: [createSuccessEmbed('⏸️ Paused the current track.')] });
        setTimeout(() => {
    ...
    async executePrefix(message: Message, args: string[]) {
        ...
        if (queue.paused) {
            const errReply = await message.reply({ embeds: [createErrorEmbed('The track is already paused!')] });
            setTimeout(() => {
                errReply.delete().catch(() => {});
                message.delete().catch(() => {});
            }, 5000);
        ...
        queue.paused = true;
        queue.player.setPaused(true);
        const reply = await message.reply({ embeds: [createSuccessEmbed('⏸️ Paused the current track.')] });
        setTimeout(() => {
            reply.delete().catch(() => {});
            message.delete().catch(() => {});
        }, 5000);
