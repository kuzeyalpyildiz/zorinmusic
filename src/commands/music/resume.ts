...
const command: ZorinCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (!queue.paused) {
            await interaction.editReply({ embeds: [createErrorEmbed('The track is not paused!')] });
            setTimeout(() => {
                interaction.deleteReply().catch(() => {});
            }, 5000);
            return;
        ...
        await interaction.editReply({ embeds: [createSuccessEmbed('▶️ Resumed the current track.')] });
        setTimeout(() => {
            interaction.deleteReply().catch(() => {});
        }, 5000);
    },
