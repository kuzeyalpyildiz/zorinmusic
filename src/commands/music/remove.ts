...
const command: ZorinCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (pos > queue.tracks.length) {
            await interaction.editReply({ embeds: [createErrorEmbed(`Invalid position! There are only ${queue.tracks.length} tracks in the queue.`)] });
            setTimeout(() => {
                interaction.deleteReply().catch(() => {});
            }, 5000);
            return;
        ...
        if (!removedTrack) {
            await interaction.editReply({ embeds: [createErrorEmbed('Failed to remove track.')] });
            setTimeout(() => {
                interaction.deleteReply().catch(() => {});
            }, 5000);
            return;
        ...
        await interaction.editReply({ embeds: [createSuccessEmbed(`Removed **${removedTrack.info.title}** from the queue.`)] });
