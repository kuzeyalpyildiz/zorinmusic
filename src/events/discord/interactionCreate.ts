...
import { createErrorEmbed, createNowPlayingEmbed, createNowPlayingComponents } from '../../utils/embeds';
...
export default {
    ...
    async execute(client: ZorinClient, interaction: Interaction) {
        ...
        if (interaction.isButton()) {
            ...
            if (!voiceChannel) {
                await interaction.reply({
                    embeds: [createErrorEmbed('You must be in a voice channel to use music controls!')],
                    ephemeral: true,
                });
                return;
            }
            ...
            if (!voiceChannel.members.has(client.user!.id)) {
                await interaction.reply({
                    embeds: [createErrorEmbed('You must be in the same voice channel as Zorin Music!')],
                    ephemeral: true,
                });
                return;
            }
            ...
            try {
                    ...
                    case 'btn_prev': {
                        ...
                        if (!prev) {
                            await interaction.followUp({
                                embeds: [createErrorEmbed('No previous track found in history!')],
                                ephemeral: true,
                            }).catch(() => {});
                            return;
                        }
                ...
                if (queue.current && queue.lastNowPlayingMessage) {
                    const updatedEmbed = createNowPlayingEmbed(queue.current, queue.player.position);
